...
function setupCors(app: express.Application) {
  app.use((req, res, next) => {
    ...
    if (process.env.YOUR_DOMAIN) {
      origins.add(`https://${process.env.YOUR_DOMAIN}`);
      origins.add(`http://${process.env.YOUR_DOMAIN}`);
    }

    // Production domains
    if (process.env.PRODUCTION_DOMAIN) {
      origins.add(`https://${process.env.PRODUCTION_DOMAIN}`);
    }
    ...
    if (origin && (origins.has(origin) || isLocalhost || isExpoOrigin || !origin)) {
    ...
    } else if (origin && !origins.has(origin) && !isLocalhost && !isExpoOrigin) {
